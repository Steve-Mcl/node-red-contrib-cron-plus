...
* Solar event schedules aren't limited to the usual presets (sunrise, sunset, dusk, dawn, golden hour etc.) - you can also fire on any custom sun angle (e.g. "when the sun is 4° below the horizon, rising") via the "Custom sun angle" control, or by setting `solarEvents` to `angle:<degrees>:rise` / `angle:<degrees>:set` directly
