...
function evalTerm (term, ctx) {
    ...
    switch (term.kind) {
        ...
        case 'clockEvery': {
            // steps are counted from LOCAL midnight so a size that does not
            // divide the hour ("every 45 minutes") is still well defined
            const p = getLocalParts(ts, tz)
            const mins = (p.hour * 60) + p.minute
            return { pass: mins % term.stepMin === 0, detail: 'local time is ' + fmtDetailTime(ts, tz) }
        }
        case 'minuteOfHour': {
            const p = getLocalParts(ts, tz)
...
// n / (its smallest prime factor): 60 -> 30, 45 -> 15, 5 -> 1, 1 -> 1
...
function pickResolution (ast) {
    ...
    ast.groups.forEach(function (g) {
        g.terms.forEach(function (t) {
            ...
            } else if (t.kind === 'clockEvery') {
                // a step matches for one minute every stepMin, so sampling AT
                // stepMin would hit nothing but matches and never see the gap:
                // the grid has to be a PROPER divisor of the step
                res = gcd(res, largestProperDivisor(t.stepMin)) || res
...
function findWindows (ast, opts) {
    ...
    // sampling stays aligned to the grid (the matches land on it), but the grid
    // point at or before `startTs` describes the PAST: sampling it directly
    // reported windows that had already closed ("deny until 12:30" at 12:42).
    // Whether we are inside a window right now is answered by startTs itself.
    let t = (Math.floor(startTs / stepMs) * stepMs) + stepMs
    const scanEnd = startTs + horizonMs
    const windows = []
    let open = passAt(startTs) ? startTs : null
    let truncated = false
    while (t < scanEnd) {
        ...
        if (pass && open === null) {
            ...
            open = refineFlip(t - stepMs, t, false)
        }
        if (!pass && open !== null) {
